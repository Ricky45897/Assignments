...
def init_gameboard():
    ...
    """
    ### TASK 1 STARTS HERE ###
    gameboard = [[0 for col in range (6)] for row in range (20)]


    return gameboard  # this line also need to change
    ### TASK 1 ENDS HERE ###
...
def default_random_number_generator():
    ...
    for random_times in range (2):
        index = random.randint(0, len(candidates) - 1)  # Generate random number from 0 to len(candidates) - 1      
        index_list.append(index)

        while index_list[random_times] == 8 or index_list[random_times] == 9:
            index_list.pop(random_times)
